...
void TdObject::send_query(td_api::object_ptr<td_api::Function> f, std::function<void(Object)> handler) {
        auto queryId = next_query_id();
        if (handler) {
            handlers_.emplace(queryId, std::move(handler));
        }
        client_manager_->send(client_id_, queryId, std::move(f));
}

void TdObject::process_response(td::ClientManager::Response response) {
        if (!response.object) {
            return;
        }
...
std::string TdObject::get_user_name(std::int64_t user_id) const {
...
std::string TdObject::get_chat_title(std::int64_t chat_id) const {
...
void TdObject::process_update(td_api::object_ptr<td_api::Object> update) {
...
void TdObject::check_authentication_error(Object object) {
      if (object->get_id() == td_api::error::ID) {
...
auto TdObject::create_authentication_query_handler() {
...
void TdObject::on_authorization_state_update() {
